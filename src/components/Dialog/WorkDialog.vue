    ...
    <v-card>
      <v-card-title>
        <v-row>
          ...
          <v-col v-if="!smallScreen" xl="4" lg="4" md="4" cols="12"
      ...
      <v-card-text class="hide-scroll">
        <v-row>
          <v-col xl="9" lg="9" md="9" sm="12" ref="img-col">
            <v-card
              class="img-card flex-center pointer-cursor"
              max-width="100%"
              min-width="100%"
              :max-height="smallScreen ? '100%' : 480"
              :min-height="smallScreen ? '100%' : 480"
              tile
              flat>
              <v-img
                :src="workData.image"
              ...
              <div class="icon-options flex-center" v-if="!smallScreen && showOverlay">
              ...
              <div class="overlay" v-if="!smallScreen && showOverlay"></div>
            </v-card>
          </v-col>
          <v-col xl="3" lg="3" md="3" sm="12" class="pl-0 pr-0">
            <v-card flat>
              <v-card-text v-if="smallScreen" class="pt-0 pb-0">
                <v-row>
...
export default {
  ...
  computed: {
    smallScreen() {
      return (/sm|xs/).test(this.$vuetify.breakpoint.name);
    }
  },
  mounted() {
    if (!this.smallScreen) {
